...
from py2joern.vulnPasses.basePass import BaseAnalysisPass, VulnerabilityFinding
...
class CWE78():
    ...
    def analysis_function(self) -> List['Source']:
        """
        ...
        for func in functions: 
            request = self.get_LLMrequest(func)
            response = self.llmBridge.analyze_function(request)
            self.reslove_response(response,func)


    def reslove_response(self, response: dict,func:Function) -> List[Source]:
        ...
        for role in roles:
            ...
            if role_type == 'SOURCE':
                source = Source.create_from_function(func, parameter_index)
                self.sources.append(source)
            elif role_type == 'SINK':
                sink = Sink.create_from_function(func, parameter_index)
    ...
    def get_LLMrequest(self, func: Function) -> LLMRequest:
        """获取函数分析的提示"""
    ...
    - 如果函数与命令执行无关 → 标记为NONE"""
        ...
        return LLMRequest(analysis_type = None,system_content=system_content,prompt=user_content)


    def taint_analysis(self) -> DataFlowResult:
        for source in self.sources:
            for sink in self.sinks:
                try:
                    dataflow_result = self.cpg.taint_analysis(source, sink)
                    self.dataFlowResults.append(dataflow_result)
                    logging.info(f"分析源 {source} 到汇聚点 {sink} 的数据流结果: {dataflow_result}")   
                    return dataflow_result                     
                except Exception as e:
                    logging.error(f"分析源 {source} 到汇聚点 {sink} 时出错: {e}")
